...
#include "../../foundation/media/include/ABitReader.h"
...
#include "../../foundation/media/include/MetaData.h"
...
void FindAVCDimensions(
    ...
    *height = (2 - frame_mbs_only_flag)
    ...
    if (br.getBits(1)) {  // frame_cropping_flag
        ...
        LLOGV("frame_crop = (%u, %u, %u, %u), cropUnitX = %u, cropUnitY = %u",
    ...
    if (br.getBits(1)) {  // vui_parameters_present_flag
        ...
        LLOGV("sample aspect ratio = %u : %u", sar_width, sar_height);
...
sp<MetaData> MakeAVCCodecSpecificData(const sp<ABuffer> &accessUnit) {
    ...
    *out++ = picParamSet->size() & 0xff;
    ...
    if (sarWidth > 1 || sarHeight > 1) {
        ...
        LLOGI("found AVC codec config (%d x %d, %s-profile level %d.%d) "
              "SAR %d : %d",
             width,
             height,
...
bool IsIDR(const sp<ABuffer> &buffer) {
    ...
    while (getNextNALUnit(&data, &size, &nalStart, &nalSize, true) == OK) {
        if (nalSize == 0u) {
            LLOGW("skipping empty nal unit from potentially malformed bitstream");
            continue;
        }
