...
#ifndef LIYL_PRIVATE_WRITEPROTECTED_H
#define LIYL_PRIVATE_WRITEPROTECTED_H
 ...
 public:
  ...
  void initialize() {
    ...
    if (mprotect(&contents, PAGE_SIZE, PROT_READ)) {
        fprintf(stdout,"failed to make WriteProtected nonwritable in initialize");
    }
  }
  ...
  void mutate(Mutator mutator) {
    if (mprotect(&contents, PAGE_SIZE, PROT_READ | PROT_WRITE) != 0) {
       fprintf(stdout,"failed to make WriteProtected writable in mutate: %s",
                   strerror(errno));
    }
    mutator(&contents.value);
    if (mprotect(&contents, PAGE_SIZE, PROT_READ) != 0) {
       fprintf(stdout,"failed to make WriteProtected nonwritable in mutate: %s",
                   strerror(errno));
    }
  }
};
