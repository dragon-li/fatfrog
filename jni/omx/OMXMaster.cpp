...
OMXMaster::OMXMaster()
    ...
    if (mProcessName[0] == 0) {
        ...
        if (fd < 0) {
            LLOGW("couldn't determine process name");
...
void OMXMaster::addPlugin(OMXPluginBase *plugin) {
    ...
    while ((err = plugin->enumerateComponents(
        ...
        if (mPluginByComponentName.indexOfKey(name8) >= 0) {
            LLOGE("A component of name '%s' already exists, ignoring this one.",
                 name8.c_str());
    ...
    if (err != OMX_ErrorNoMore) {
        LLOGE("OMX plugin failed w/ error 0x%08x after registering %zu "
...
OMX_ERRORTYPE OMXMaster::makeComponentInstance(
    ...
    LLOGI("makeComponentInstance(%s) in %s process", name, mProcessName);
    Mutex::Autolock autoLock(mLock);

    *component = NULL;

    ssize_t index = mPluginByComponentName.indexOfKey(AString(name));
...
OMX_ERRORTYPE OMXMaster::enumerateComponents(
    ...
    const AString &name8 = mPluginByComponentName.keyAt(index);
