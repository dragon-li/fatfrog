...
#include "../foundation/media/include/ALooper.h"
    ...
    switch (index) {
        case OMX_IndexParamPortDefinition:
        {
            ...
            if (defParams->nBufferCountActual < port->mDef.nBufferCountMin) {
                LLOGW("component requires at least %u buffers (%u requested)",
...
void SimpleSoftOMXComponent::onChangeState(OMX_STATETYPE state) {
    LLOGV("%p requesting change from %d to %d", this, mState, state);
...
void SimpleSoftOMXComponent::onPortEnable(OMX_U32 portIndex, bool enable) {
    ...
    if (port->mDef.eDir != OMX_DirOutput) {
        LLOGE("Port enable/disable allowed only on output ports.");
...
void SimpleSoftOMXComponent::checkTransitions() {
    if (mState != mTargetState) {
        ...
        if (transitionComplete) {
            LLOGV("state transition from %d to %d complete", mState, mTargetState);
            mState = mTargetState;
            ...
            LLOGV("state transition from %d to %d not yet complete", mState, mTargetState);
        }
    }
