...
LIYL_NAMESPACE_START
    ...
    status_t AReplyToken::setReply(const sp<AMessage> &reply) {
        if (mReplied) {
            LLOGE("trying to post a duplicate reply");
            return -EBUSY;
        }
        CHECK(mReply == NULL);
        mReply = reply;
...
#include <utils/Mutex.h>
    ...
    static void reportStats() {
        ...
        if (time / 1000 != gLastChecked / 1000) {
            gLastChecked = time;
            LLOGI("called findItemIx %zu times (for len=%.1f i=%.1f/%.1f mem) dup %zu times (for len=%.1f)",
                    gFindItemCalls,
...
#undef BASIC_TYPE
    ...
    void AMessage::deliver() {
        ...
        if (handler == NULL) {
            LLOGW("failed to deliver message as target handler %d is gone.", mTarget);
            return;
        }
    ...
    status_t AMessage::post(int64_t delayUs) {
        ...
        if (looper == NULL) {
            LLOGW("failed to post message as target looper for handler %d is gone.", mTarget);
            return -ENOENT;
        }
    ...
    status_t AMessage::postAndAwaitResponse(sp<AMessage> *response) {
        ...
        if (token == NULL) {
            LLOGE("failed to create reply token");
            return -ENOMEM;
        }
    ...
    status_t AMessage::postReply(const sp<AReplyToken> &replyToken) {
        if (replyToken == NULL) {
            LLOGW("failed to post reply to a NULL token");
            return -ENOENT;
        }
        sp<ALooper> looper = replyToken->getLooper();
        if (looper == NULL) {
            LLOGW("failed to post reply as target looper is gone.");
            return -ENOENT;
        }
    ...
    sp<AMessage> AMessage::changesFrom(const sp<const AMessage> &other, bool deep) const {
        ...
        for (size_t i = 0; i < mNumItems; ++i) {
            ...
            switch (item.mType) {
                ...
                default:
                    {
                        LLOGE("Unknown type %d", item.mType);
                        TRESPASS();
                    }
            }
        }
