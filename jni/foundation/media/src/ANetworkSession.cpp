...
LIYL_NAMESPACE_START
    ...
    ANetworkSession::Session::~Session() {
        LLOGV("Session %d gone", mSessionID);
        ...
        if (mState == DATAGRAM) {
            ...
            if (err != OK) {
                if (!mUDPRetries) {
                    ...
                    mUDPRetries--;
                    LLOGE("Recvfrom failed, %d/%d retries left",
                            mUDPRetries, kMaxUDPRetries);
                    err = OK;
                }
            } else {
    ...
    status_t ANetworkSession::Session::writeMore() {
        if (mState == DATAGRAM) {
            ...
            if (err == -EAGAIN) {
                if (!mOutFragments.empty()) {
                    LLOGI("%zu datagrams remain queued.", mOutFragments.size());
                }
                err = OK;
            }
            ...
            if (err != OK) {
                if (!mUDPRetries) {
                    ...
                    mUDPRetries--;
                    LLOGE("Send datagram failed, %d/%d retries left",
                            mUDPRetries, kMaxUDPRetries);
                    err = OK;
                }
            } else {
        ...
        if (res == 0 && numBytesQueued > 50 * 1024) {
            if (numBytesQueued > 409600) {
                LLOGW("!!! numBytesQueued = %d", numBytesQueued);
            }
    ...
    status_t ANetworkSession::start() {
        ...
        status_t err = mThread->run("ANetworkSession", LIYL_PRIORITY_AUDIO);
    ...
    status_t ANetworkSession::createClientOrServer(
        ...
        *sessionID = 0;
        ...
        if (mode == kModeCreateRTSPClient
            ...
            LLOGI("connecting socket %d to %d.%d.%d.%d:%d",
                    s,
                    (x >> 24),
                    (x >> 16) & 0xff,
                    (x >> 8) & 0xff,
        ...
        if(mSessions.insert(tSessionsPair(session->sessionID(), session)).second == false) {
            LLOGE("mSessions insert failed");
        }
        interrupt();
...
bail:
    ...
    status_t ANetworkSession::connectUDPSession(
        ...
        const sp<Session> session = mSessions[sessionID];
    ...
    void ANetworkSession::interrupt() {
        ...
        if (n < 0) {
            LLOGW("Error writing to pipe (%s)", strerror(errno));
        }
    }
    ...
    void ANetworkSession::threadLoop() {
...

        {
            ...
            for (tSessionsMap::iterator it = mSessions.begin();it != mSessions.end();it++) {
        ...
        if (res < 0) {
            ...
            LLOGE("select failed w/ error %d (%s)", errno, strerror(errno));
            return;
        }
        ...
        if (FD_ISSET(mPipeFd[0], &rs)) {
            ...
            if (n < 0) {
                LLOGW("Error reading from pipe (%s)", strerror(errno));
            }
...

        {
            ...
            for (tSessionsMap::iterator it = mSessions.begin();it != mSessions.end() && res > 0;it++) {
                const sp<Session> &session = it->second; 
                int s = session->socket();
                ...
                if (FD_ISSET(s, &rs)) {
                    if (session->isRTSPServer() || session->isTCPDatagramServer()) {
                        ...
                        if (clientSocket >= 0) {
                            ...
                            if (err != OK) {
                                LLOGE("Unable to make client socket non blocking, "
                                        "failed w/ error %d (%s)",
                                        err, strerror(-err));
                                ...
                                LLOGI("incoming connection from %d.%d.%d.%d:%d "
                                        "(socket %d)",
                                        (addr >> 24),
                                        (addr >> 16) & 0xff,
                                        (addr >> 8) & 0xff,
                            ...
                            LLOGE("accept returned error %d (%s)",
                                    errno, strerror(errno));
                        }
                    } else {
                        status_t err = session->readMore();
                        if (err != OK) {
                            LLOGE("readMore on socket %d failed w/ error %d (%s)",
                                    s, err, strerror(-err));
                        }
                    }
                }
                ...
                if (FD_ISSET(s, &ws)) {
                    ...
                    if (err != OK) {
                        LLOGE("writeMore on socket %d failed w/ error %d (%s)",
                                s, err, strerror(-err));
                    }
                }
            }
            ...
            while (!sessionsToAdd.empty()) {
                ...
                LLOGI("added clientSession %d", session->sessionID());
            }
        }
    }
