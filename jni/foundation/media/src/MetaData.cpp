...
void *MetaData::typed_data::allocateStorage(size_t size) {
    ...
    if (u.ext_data == NULL) {
        LLOGE("Couldn't allocate %zu bytes for item", size);
        mSize = 0;
    }
    return u.ext_data;
}
...
AString MetaData::typed_data::asString(bool verbose) const {
    AString out;
    const void *data = storage();
    switch(mType) {
        case TYPE_NONE:
            out = AStringPrintf("no type, size %zu)", mSize);
            break;
        case TYPE_C_STRING:
            out = AStringPrintf("(char*) %s", (const char *)data);
            break;
        case TYPE_INT32:
            out = AStringPrintf("(int32_t) %d", *(int32_t *)data);
            break;
        case TYPE_INT64:
            out = AStringPrintf("(int64_t) %" PRId64, *(int64_t *)data);
            break;
        case TYPE_FLOAT:
            out = AStringPrintf("(float) %f", *(float *)data);
            break;
        case TYPE_POINTER:
            out = AStringPrintf("(void*) %p", *(void **)data);
            break;
        case TYPE_RECT:
        {
            const Rect *r = (const Rect *)data;
            out = AStringPrintf("Rect(%d, %d, %d, %d)",
                                  r->mLeft, r->mTop, r->mRight, r->mBottom);
            break;
        }

        default:
            out = AStringPrintf("(unknown type %d, size %zu)", mType, mSize);
...
AString MetaData::toString() const {
    AString s;
    for (int i = mItems.size(); --i >= 0;) {
        ...
        const typed_data &item = mItems.valueAt(i);
        s = AStringPrintf("%s: %s", cc, item.asString(false).c_str());
        if (i != 0) {
            s.append(", ");
        }
    }
...
void MetaData::dumpToLog() const {
    for (int i = mItems.size(); --i >= 0;) {
        ...
        MakeFourCCString(key, cc);
        const typed_data &item = mItems.valueAt(i);
        LLOGI("%s: %s", cc, item.asString(true /* verbose */).c_str());
    }
}
