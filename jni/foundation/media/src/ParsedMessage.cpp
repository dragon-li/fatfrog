...
LIYL_NAMESPACE_START
    ...
    ssize_t ParsedMessage::parse(const char *data, size_t size, bool noMoreData) {
        ...
        std::pair<AString,AString> initDict("liyl","liyl");
        std::pair<AString,AString>& lastDict = initDict;
        ...
        while (offset < size) {
            ...
            if (offset == 0) {
                ...
                if(mDict.insert(std::pair<AString,AString>(AString("_"), line)).second == false) {
                    LLOGE("insert failed");
                }
            ...
            if (line.c_str()[0] == ' ' || line.c_str()[0] == '\t') {
                ...
                if (!lastDict.second.equalsIgnoreCase(AString("liyl"))) {
                    // Otherwise it's malformed since the first header line
                    // cannot continue anything...

                    AString &value = lastDict.second;
                    value.append(line);
                }
            ...
            if (colonPos >= 0) {
                ...
                lastDict = *(mDict.insert(std::pair<AString,AString>(key, line)).first);
            }
        ...
        for (tDictMap::iterator it=mDict.begin(); it!=mDict.end(); ++it) {
            it->second.trim();
        }
    ...
    AString ParsedMessage::debugString() const {
        ...
        for (tDictMap::const_iterator it = mDict.cbegin(); it != mDict.cend(); ++it) {
            const AString &key = it->first;
            const AString &value = it->second;
