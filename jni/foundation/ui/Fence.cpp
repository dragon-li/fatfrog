...
#include "../system/core/sync/include/sync/sync.h"
...
status_t Fence::waitForever(const char* logname) {
    ...
    if (err < 0 && errno == ETIME) {
        LLOGE("%s: fence %d didn't signal in %u ms", logname, mFenceFd,
                warningTimeout);
...
sp<Fence> Fence::merge(const AString& name, const sp<Fence>& f1,
        const sp<Fence>& f2) {
    int result;
    ...
    if (f1->isValid() && f2->isValid()) {
        result = sync_merge(name.c_str(), f1->mFenceFd, f2->mFenceFd);
    } else if (f1->isValid()) {
        result = sync_merge(name.c_str(), f1->mFenceFd, f1->mFenceFd);
    } else if (f2->isValid()) {
        result = sync_merge(name.c_str(), f2->mFenceFd, f2->mFenceFd);
    } else {
        return NO_FENCE;
    }
    if (result == -1) {
        status_t err = -errno;
        LLOGE("merge: sync_merge(\"%s\", %d, %d) returned an error: %s (%d)",
                name.c_str(), f1->mFenceFd, f2->mFenceFd,
                strerror(-err), err);
        return NO_FENCE;
    }
...
nsecs_t Fence::getSignalTime() const {
    ...
    if (finfo == NULL) {
        LLOGE("sync_fence_info returned NULL for fd %d", mFenceFd);
        return -1;
    }
    if (finfo->status != 1) {
