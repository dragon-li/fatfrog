...
#include <inttypes.h>
#include <arpa/inet.h>
#include "../../../media/include/ABuffer.h"
#include "../../../media/include/ADebug.h"
#include "../../../media/include/ALookup.h"
...
static ALookup<liyl_dataspace, liyl_dataspace> sLegacyDataSpaceToV0 {
    {
...
bool ColorUtils::convertDataSpaceToV0(liyl_dataspace &dataSpace) {
...
// static
liyl_dataspace ColorUtils::getDataSpaceForColorAspects(ColorAspects &aspects, bool mayExpand) {
    ...
    if (aspects.mRange == ColorAspects::RangeUnspecified
            ...
            || aspects.mMatrixCoeffs == ColorAspects::MatrixUnspecified
            || aspects.mTransfer == ColorAspects::TransferUnspecified) {
        LLOGW("expected specified color aspects (%u:%u:%u:%u)",
    ...
    liyl_dataspace dataSpace = (liyl_dataspace)(
...
void ColorUtils::getColorAspectsFromFormat(const sp<AMessage> &format, ColorAspects &aspects) {
    ...
    if (convertPlatformColorAspectsToCodecAspects(
        ...
        LLOGW("Ignoring illegal color aspects(R:%d(%s), S:%d(%s), T:%d(%s))",
    ...
    LLOGV("Got color aspects (R:%d(%s), P:%d(%s), M:%d(%s), T:%d(%s)) "
...
void ColorUtils::setColorAspectsIntoFormat(
    ...
    LLOGV("Setting color aspects (R:%d(%s), P:%d(%s), M:%d(%s), T:%d(%s)) "
...
bool ColorUtils::getHDRStaticInfoFromFormat(const sp<AMessage> &format, HDRStaticInfo *info) {
    ...
    if (buf->size() != 25 /* static Metadata Type 1 size */) {
        LLOGW("Ignore invalid HDRStaticInfo with size: %zu", buf->size());
        return false;
    }

    const uint8_t *data = buf->data();
    if (*data != HDRStaticInfo::kType1) {
        LLOGW("Unsupported static Metadata Type %u", *data);
        return false;
    }
    ...
    LLOGV("Got HDRStaticInfo from config (R: %u %u, G: %u %u, B: %u, %u, W: %u, %u, "
