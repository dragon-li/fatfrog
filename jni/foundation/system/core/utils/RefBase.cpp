...
#define INITIAL_STRONG_VALUE (1<<28)
                  ...
                  , mRetain(false)
        {
            ...
            ~weakref_impl()
            {
                ...
                if (!mRetain && mStrongRefs != NULL) {
                    dumpStack = true;
                    LLOGE("Strong references remain:");
                    ref_entry* refs = mStrongRefs;
                    while (refs) {
                        char inc = refs->ref >= 0 ? '+' : '-';
                        LLOGD("\t%c ID %p (ref %d):", inc, refs->id, refs->ref);
                ...
                if (dumpStack) {
                    LLOGE("above errors at:");
                    CallStack stack(LOG_TAG);
                }
            }

            void addStrongRef(const void* id) {
                //LLOGD_IF(mTrackEnabled,
            ...
            void printRefs() const
            {
...

                {
                    ...
                    if (rc >= 0) {
                        ...
                        close(rc);
                        LLOGD("STACK TRACE for %p saved in %s", this, name);
                    }
                    else LLOGE("FAILED TO PRINT STACK TRACE for %p in %s: %s", this,
                            name, strerror(errno));
                }
            }
        ...
        private:
            ...
            void removeRef(ref_entry** refs, const void* id)
            {
                if (mTrackEnabled) {
                    ...
                    LLOGE("RefBase: removing id %p on RefBase %p"
                    ...
                    while (ref) {
                        char inc = ref->ref >= 0 ? '+' : '-';
                        LLOGD("\t%c ID %p (ref %d):", inc, ref->id, ref->ref);
                        ref = ref->next;
                    }
    ...
    void RefBase::incStrong(const void* id) const
    {
        ...
        LLOG_ASSERT(c > 0, "incStrong() called on %p after last strong ref", refs);
#if PRINT_REFS
        LLOGD("incStrong of %p from %p: cnt=%d\n", this, id, c);
#endif
        ...
        LLOG_ASSERT(old > INITIAL_STRONG_VALUE, "0x%x too small", old);
...
#if PRINT_REFS
        LLOGD("decStrong of %p from %p: cnt=%d\n", this, id, c);
#endif
        LLOG_ASSERT(c >= 1, "decStrong() called on %p too many times", refs);
        if (c == 1) {
    ...
    void RefBase::forceIncStrong(const void* id) const
    {
        ...
        refs->addStrongRef(id);
        const int32_t c = refs->mStrong.fetch_add(1, std::memory_order_relaxed);
        LLOG_ASSERT(c >= 0, "forceIncStrong called on %p after ref count underflow",
                refs);
#if PRINT_REFS
        LLOGD("forceIncStrong of %p from %p: cnt=%d\n", this, id, c);
#endif
    ...
    void RefBase::weakref_type::incWeak(const void* id)
    {
        ...
        const int32_t c __unused = impl->mWeak.fetch_add(1,
                std::memory_order_relaxed);
        LLOG_ASSERT(c >= 0, "incWeak called on %p after last weak ref", this);
    }
    ...
    void RefBase::weakref_type::decWeak(const void* id)
    {
        ...
        LLOG_ASSERT(c >= 1, "decWeak called on %p too many times", this);
        if (c != 1) return;
        ...
        if ((flags&OBJECT_LIFETIME_MASK) == OBJECT_LIFETIME_STRONG) {
            ...
            if (impl->mStrong.load(std::memory_order_relaxed)
                ...
                // LLOGV("Freeing refs %p of old RefBase %p\n", this, impl->mBase);
                delete impl;
            }
        } else {
...
#if PRINT_REFS
        LLOGD("attemptIncStrong of %p from %p: cnt=%d\n", this, id, curCount);
#endif
    ...
    bool RefBase::weakref_type::attemptIncWeak(const void* id)
    {
        ...
        LLOG_ASSERT(curCount >= 0, "attemptIncWeak called on %p after underflow",
                this);
        while (curCount > 0) {
