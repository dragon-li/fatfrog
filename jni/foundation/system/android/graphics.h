...
#ifndef SYSTEM_CORE_INCLUDE_LIYL_GRAPHICS_H
#define SYSTEM_CORE_INCLUDE_LIYL_GRAPHICS_H
...
typedef enum liyl_pixel_format {
    /*
     ...
     *  HAL_DATASPACE_DEPTH           | An liyl_depth_points buffer
     *  Other                         | Unsupported
     *
     */
     ...
     *
     * struct liyl_ycbcr (below) is the the struct used to describe it.
     *
 ...
 * neighboring pixels.
 *
 * The liyl_flex_layout structure is used with any pixel format that can be
...
typedef enum liyl_flex_component {
    /* luma */
...
} liyl_flex_component_t;

typedef struct liyl_flex_plane {
    /* pointer to the first byte of the top-left pixel of the plane. */
    uint8_t *top_left;

    liyl_flex_component_t component;
...
typedef struct liyl_flex_layout {
    /* the kind of flexible format */
    liyl_flex_format_t format;
 ...
 *
 *   For example:
 *     liyl_depth_points d = get_depth_buffer();
 *     struct {
...
typedef enum liyl_dataspace {
     ...
     *       The point cloud will be represented with the liyl_depth_points
     *       structure.
     */
 ...
 * display.
 */
typedef enum liyl_color_transform {
...
#endif /* SYSTEM_CORE_INCLUDE_LIYL_GRAPHICS_H */
