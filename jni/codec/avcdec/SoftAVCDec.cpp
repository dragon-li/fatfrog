...
static size_t GetCPUCoreCount() {
    ...
    LLOGV("Number of CPU cores: %ld", cpuCoreCount);
...
void SoftAVC::logVersion() {
    ...
    if (status != IV_SUCCESS) {
        LLOGE("Error in getting version number: 0x%x",
                s_ctl_op.u4_error_code);
    } else {
        LLOGV("Ittiam decoder version number: %s",
...
status_t SoftAVC::setParams(size_t stride) {
    ...
    LLOGV("Set the run-time (dynamic) parameters stride = %zu", stride);
    status = ivdec_api_function(mCodecCtx, (void *)&s_ctl_ip, (void *)&s_ctl_op);

    if (status != IV_SUCCESS) {
        LLOGE("Error in setting the run-time parameters: 0x%x",
                s_ctl_op.u4_error_code);
...
status_t SoftAVC::resetDecoder() {
    ...
    if (IV_SUCCESS != status) {
        LLOGE("Error in reset: 0x%x", s_ctl_op.u4_error_code);
        return UNKNOWN_ERROR;
    }
    mSignalledError = false;
...
status_t SoftAVC::setNumCores() {
    ...
    if (IV_SUCCESS != status) {
        LLOGE("Error in setting number of cores: 0x%x",
...
status_t SoftAVC::setFlushMode() {
    ...
    if (status != IV_SUCCESS) {
        LLOGE("Error in setting the decoder in flush mode: (%d) 0x%x", status,
...
bool SoftAVC::getVUIParams() {
    ...
    if (status != IV_SUCCESS) {
        LLOGW("Error in getting VUI params: 0x%x",
...
void SoftAVC::onPortFlushCompleted(OMX_U32 portIndex) {
    ...
    if (kOutputPortIndex == portIndex) {
        ...
        if (NULL == mFlushOutBuffer) {
            LLOGE("Could not allocate flushOutputBuffer of size %u", bufferSize);
            return;
        }
...
void SoftAVC::onQueueFilled(OMX_U32 portIndex) {
    ...
    if (NULL == mCodecCtx) {
        if (OK != initDecoder()) {
            LLOGE("Failed to initialize decoder");
    ...
    while (!outQueue.empty()) {
...

        {
            ...
            if (!setDecodeArgs(&s_dec_ip, &s_dec_op, inHeader, outHeader, timeStampIx)) {
                LLOGE("Decoder arg setup failed");
            ...
            if (unsupportedResolution) {
                LLOGE("Unsupported resolution : %dx%d", mWidth, mHeight);
                notify(OMX_EventError, OMX_ErrorUnsupportedSetting, 0, NULL);
                mSignalledError = true;
                return;
            }
            ...
            if (allocationFailed) {
                LLOGE("Allocation failure in decoder");
                notify(OMX_EventError, OMX_ErrorUnsupportedSetting, 0, NULL);
                mSignalledError = true;
                return;
            }
...
liyl::SoftOMXComponent *createSoftOMXComponent(
        ...
        OMX_COMPONENTTYPE **component) {
    return new liyl::SoftAVC(name, callbacks, appData, component);
}
