...
status_t SoftAAC2::initDecoder() {
    ...
    if (mAACDecoder == NULL) {
        LLOGE("AAC decoder is null. TODO: Can not call aacDecoder_SetParam in the following code");
    }
...
OMX_ERRORTYPE SoftAAC2::internalGetParameter(
    ...
    switch ((OMX_U32) index) {
        ...
        {
            OMX_AUDIO_PARAM_LIYL_PROFILETYPE *profileParams =
                (OMX_AUDIO_PARAM_LIYL_PROFILETYPE *)params;
...
OMX_ERRORTYPE SoftAAC2::internalSetParameter(
    ...
    switch ((int)index) {
        ...
        case OMX_IndexParamAudioLiylAacPresentation:
        {
            const OMX_AUDIO_PARAM_LIYL_AACPRESENTATIONTYPE *aacPresParams =
                    (const OMX_AUDIO_PARAM_LIYL_AACPRESENTATIONTYPE *)params;
...
bool SoftAAC2::outputDelayRingBufferPutSamples(INT_PCM *samples, int32_t numSamples) {
    ...
    if (outputDelayRingBufferSpaceLeft() < numSamples) {
        LLOGE("RING BUFFER WOULD OVERFLOW");
        return false;
    }
    if (mOutputDelayRingBufferWritePos + numSamples <= mOutputDelayRingBufferSize
        ...
        LLOGV("slow SoftAAC2::outputDelayRingBufferPutSamples()");
...
int32_t SoftAAC2::outputDelayRingBufferGetSamples(INT_PCM *samples, int32_t numSamples) {
    ...
    if (numSamples > mOutputDelayRingBufferFilled) {
        LLOGE("RING BUFFER WOULD UNDERRUN");
        return -1;
    }
    ...
    if (mOutputDelayRingBufferReadPos + numSamples <= mOutputDelayRingBufferSize
        ...
        LLOGV("slow SoftAAC2::outputDelayRingBufferGetSamples()");
    ...
    while ((!inQueue.empty() || mEndOfInput) && !outQueue.empty()) {
        if (!inQueue.empty()) {
            ...
            if (mInputBufferCount == 0 && !(inHeader->nFlags & OMX_BUFFERFLAG_CODECCONFIG)) {
                LLOGE("first buffer should have OMX_BUFFERFLAG_CODECCONFIG set");
            ...
            if ((inHeader->nFlags & OMX_BUFFERFLAG_CODECCONFIG) != 0) {
                ...
                if (decoderErr != AAC_DEC_OK) {
                    LLOGW("aacDecoder_ConfigRaw decoderErr = 0x%4.4x", decoderErr);
                ...
                if (mStreamInfo->sampleRate && mStreamInfo->numChannels) {
                    LLOGI("Initially configuring decoder: %d Hz, %d channels",
            ...
            if (mIsADTS) {
                ...
                if (inHeader->nFilledLen < 7) {
                    LLOGE("Audio data too short to contain even the ADTS header. "
                    ...
                    if (inHeader->nFilledLen < aac_frame_length) {
                        LLOGE("Not enough audio data for the complete frame. "
            ...
            do {
                if (outputDelayRingBufferSpaceLeft() <
                    ...
                    LLOGV("skipping decode: not enough space left in ringbuffer");
                    break;
                }
                ...
                if (decoderErr != AAC_DEC_OK) {
                    LLOGW("aacDecoder_DecodeFrame decoderErr = 0x%4.4x", decoderErr);
                }

                if (bytesValid[0] != 0) {
                    LLOGE("bytesValid[0] != 0 should never happen");
                    mSignalledError = true;
                ...
                if (decoderErr == AAC_DEC_OK) {
                    ...
                    LLOGW("AAC decoder returned error 0x%4.4x, substituting silence", decoderErr);
                ...
                if (!mStreamInfo->sampleRate || !mStreamInfo->numChannels) {
                    ...
                    LLOGI("Reconfiguring decoder: %d->%d Hz, %d->%d channels",
                ...
                if (inHeader && inHeader->nFilledLen == 0) {
                    ...
                    LLOGV("inHeader->nFilledLen = %d", inHeader ? inHeader->nFilledLen : 0);
                }
        ...
        while (!outQueue.empty()
            ...
            if (outHeader->nOffset != 0) {
                LLOGE("outHeader->nOffset != 0 is not handled");
                mSignalledError = true;
                notify(OMX_EventError, OMX_ErrorUndefined, 0, NULL);
                return;
            }
            ...
            if (available) {
                ...
                LLOGV("%d samples available (%d), or %d frames",
                ...
                for (int i = 0; i < numFrames; i++) {
                    ...
                    LLOGV("decoded %d of %d", decodedSize, *currentBufLeft);
                    if (*currentBufLeft > decodedSize) {
                        // adjust/interpolate next time stamp
                        *currentBufLeft -= decodedSize;
                        *nextTimeStamp += mStreamInfo->aacSamplesPerFrame *
                                1000000ll / mStreamInfo->aacSampleRate;
                        LLOGV("adjusted nextTimeStamp/size to %lld/%d",
                        ...
                        if (mBufferTimestamps.size() > 0) {
                            ...
                            LLOGV("moved to next time/size: %lld/%d",
                                    (long long) *nextTimeStamp, *currentBufLeft);
                        }
                ...
                LLOGV("getting %d from ringbuffer", numSamples);
                int32_t ns = outputDelayRingBufferGetSamples(outBuffer, numSamples);
                if (ns != numSamples) {
                    LLOGE("not a complete frame of samples available");
                    mSignalledError = true;
                    notify(OMX_EventError, OMX_ErrorUndefined, 0, NULL);
                    return;
                }
            ...
            outInfo = NULL;
            LLOGV("out timestamp %lld / %d", outHeader->nTimeStamp, outHeader->nFilledLen);
...
void SoftAAC2::onPortFlushCompleted(OMX_U32 portIndex) {
    if (portIndex == 0) {
        ...
        while ((avail = outputDelayRingBufferSamplesAvailable()) > 0) {
            ...
            if (ns != avail) {
                LLOGW("not a complete frame of samples available");
                break;
            }
            mOutputBufferCount++;
        }
...
liyl::SoftOMXComponent *createSoftOMXComponent(
        const char *name, const OMX_CALLBACKTYPE *callbacks,
        OMX_PTR appData, OMX_COMPONENTTYPE **component) {
    return new liyl::SoftAAC2(name, callbacks, appData, component);
}
