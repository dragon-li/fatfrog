...
#include "../../../jni/libc_bionic/dlmalloc/dlmalloc.h"
...
TEST_F(MallocDebugTest, front_guard_corrupted) {
  ...
  expected_log += LIYL::BASE::StringPrintf(
...
TEST_F(MallocDebugTest, leak_track_no_frees) {
  ...
  ASSERT_STREQ("", getFakeLogBuf().c_str());
  std::string expected_log = LIYL::BASE::StringPrintf(
        "6 malloc_debug +++ omx_test leaked block of size 1024 at %p (leak 1 of 3)\n",
      pointer3);
  expected_log += LIYL::BASE::StringPrintf(
        "6 malloc_debug +++ omx_test leaked block of size 200 at %p (leak 2 of 3)\n",
      pointer1);
  expected_log += LIYL::BASE::StringPrintf(
        "6 malloc_debug +++ omx_test leaked block of size 128 at %p (leak 3 of 3)\n",
      pointer2);
  ASSERT_STREQ(expected_log.c_str(), getFakeLogPrint().c_str());
}
...
TEST_F(MallocDebugTest, leak_track_no_frees_with_backtrace) {
  ...
  expected_log += LIYL::BASE::StringPrintf(
      "6 malloc_debug +++ omx_test leaked block of size 128 at %p (leak 2 of 3)\n",
      pointer2);
  expected_log += "6 malloc_debug Backtrace at time of allocation:\n";
  ...
  expected_log += LIYL::BASE::StringPrintf(
      "6 malloc_debug +++ omx_test leaked block of size 100 at %p (leak 3 of 3)\n",
      pointer1);
  expected_log += "6 malloc_debug Backtrace at time of allocation:\n";
...
TEST_F(MallocDebugTest, leak_track_frees) {
  ...
  ASSERT_STREQ("", getFakeLogBuf().c_str());
  std::string expected_log = LIYL::BASE::StringPrintf(
      "6 malloc_debug +++ omx_test leaked block of size 999 at %p (leak 1 of 2)\n",
      pointer3);
  expected_log += LIYL::BASE::StringPrintf(
      "6 malloc_debug +++ omx_test leaked block of size 100 at %p (leak 2 of 2)\n",
      pointer1);
  ASSERT_STREQ(expected_log.c_str(), getFakeLogPrint().c_str());
}
...
TEST_F(MallocDebugTest, free_track_use_after_free) {
  ...
  ASSERT_STREQ("", getFakeLogBuf().c_str());
  std::string expected_log(DIVIDER);
  expected_log += LIYL::BASE::StringPrintf("6 malloc_debug +++ ALLOCATION %p USED AFTER FREE\n", pointers[0]);
...
TEST_F(MallocDebugTest, backtrace_enable_on_signal) {
  ...
  ASSERT_STREQ("", getFakeLogBuf().c_str());
  std::string expected_log = LIYL::BASE::StringPrintf(
      "4 malloc_debug omx_test: Run: 'kill -%d %d' to enable backtracing.\n",
      SIGRTMIN + 10, getpid());
  ASSERT_STREQ(expected_log.c_str(), getFakeLogPrint().c_str());
}
